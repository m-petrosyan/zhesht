...
const props = defineProps({
  pastTours: {required: true, type: Object}, // Ожидаем объект
});

const sortedPastTours = computed(() => {
  // Преобразуем объект в массив пар и сортируем по убыванию
  return Object.entries(props.pastTours)
...
<template>
  <GuestLayout>
    <section class="bg-main flex flex-col gap-y-10 pt-20 pb-20">
      ...
      <div v-for="[year, data] in sortedPastTours"
           :key="year"
           class="timebar flex gap-x-5 gap-y-10"
           :ref="el => setRefs(el, year)">
        <div class="content-center text-white text-sm">
          <p :class="['inline-block rotate-90 pl-6 top-20 transition-all duration-300',
