        ...
        <section class="bg-main lg:px-8 px-6 pt-20 pb-20 min-h-[800px]">
            <div class="mt-20 lg:w-7/12 md:w-full mx-auto">
                ...
                <div class="lg:w-full mt-4">
                    <div
                        :style="{ backgroundImage: `url(${tour.poster.large})` }"
                        class="event-image w-full h-96 bg-cover bg-no-repeat bg-center z-10  rounded-lg"
                    />
                </div>
                <div class="flex lg:flex-row flex-col gap-8 mb-10">
                    ...
                    <div class="lg:w-1/2 flex flex-col gap-y-5">
                        ...
                        <div class="text-2xl">
                            <p>
                                <b>{{ tour.event?.location }}<br>
                                    {{ formatDateTime(tour.event.date_time, 'D MMMM YYYY HH:mm') }}</b>
                            </p>
                        </div>

                        <div v-if="tour.event?.tickets.length" class="flex items-center gap-x-10">
                            <div v-if="tour.event?.tickets.length > 1">
                                <button
                                ...
                                <div v-if="openTickets" class="flex flex-col gap-y-2 mt-2">
                                    <a
                                        v-for="item in tour.event?.tickets"
                                        class="bg-black text-white py-2 px-5 rounded-3xl text-lg"
                                        target="_blank"
                                        :href="item.url"
                                    >
